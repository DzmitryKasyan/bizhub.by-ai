...
$pageTitle = $currentType ? ($typeLabels[$currentType] ?? 'Каталог объявлений') : 'Каталог объявлений';
$metaDesc = $currentType
    ? "{$pageTitle} в Беларуси. Актуальные предложения, проверенные продавцы, удобный поиск."
    : 'Каталог объявлений о продаже и покупке бизнеса в Беларуси. Фильтрация по типу, категории, цене и городу.';
@endphp
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
                ...
                <div class="flex flex-wrap gap-2">
                    @if(request('type'))
                        <span class="inline-flex items-center gap-1.5 bg-primary-50 text-primary-700 text-xs px-3 py-1.5 rounded-full font-medium">
                            {{ $typeLabels[request('type')] ?? request('type') }}
                            <a href="{{ request()->fullUrlWithQuery(['type' => null]) }}" class="hover:text-blue-900">×</a>
                        </span>
                    @endif
                    @if(request('location'))
                        <span class="inline-flex items-center gap-1.5 bg-slate-100 text-slate-700 text-xs px-3 py-1.5 rounded-full font-medium">
                            {{ request('location') }}
                            <a href="{{ request()->fullUrlWithQuery(['location' => null]) }}" class="hover:text-slate-900">×</a>
                        </span>
                    @endif
                    @if(request('search'))
                        <span class="inline-flex items-center gap-1.5 bg-slate-100 text-slate-700 text-xs px-3 py-1.5 rounded-full font-medium">
                            «{{ request('search') }}»
                            <a href="{{ request()->fullUrlWithQuery(['search' => null]) }}" class="hover:text-slate-900">×</a>
                        </span>
                    @endif
                </div>
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    <label class="text-sm text-slate-500 whitespace-nowrap">Сортировка:</label>
                    <form method="GET" action="{{ route('listings.index') }}" id="sortForm">
                        ...
                        <select name="sort"
                                ...
                                class="text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-primary-300 bg-white text-slate-700">
            ...
            @if(isset($listings) && $listings->count())
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                    ...
                    <article class="bg-white rounded-2xl overflow-hidden border border-slate-100 hover:shadow-xl hover:shadow-primary-500/10 hover:border-primary-100 transition-all group card-3d">
                        <!-- Image -->
                        <a href="{{ route('listings.show', $listing->slug) }}" class="block relative overflow-hidden bg-slate-100" style="aspect-ratio: 16/10;">
                        ...
                        <div class="p-4">
                            <a href="{{ route('listings.show', $listing->slug) }}"
                               class="block font-semibold text-slate-900 hover:text-primary-600 transition-colors leading-snug mb-2 line-clamp-2 text-sm">
                                {{ $listing->title }}
                            </a>
            ...
            @else
                <div class="bg-white rounded-xl border border-slate-100 p-16 text-center">
                    ...
                    <h3 class="text-lg font-semibold text-slate-900 mb-2">Объявления не найдены</h3>
                    <p class="text-slate-500 text-sm mb-5">Попробуйте изменить параметры фильтрации</p>
                    <a href="{{ route('listings.index') }}"
                       class="inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium px-5 py-2.5 rounded-lg transition-colors">
                        Сбросить фильтры
                    </a>
                </div>
            @endif
