...
$statusConfig = [
    'draft'    => ['label' => 'Черновики',     'class' => 'bg-slate-100 text-slate-500'],
    ...
    'archived' => ['label' => 'Архив',         'class' => 'bg-slate-100 text-slate-400'],
...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-slate-900">Мои объявления</h1>
        @if(isset($listings))
            <p class="text-slate-500 text-sm mt-1">Всего: {{ $listings->total() }}</p>
        @endif
    </div>
    <a href="{{ route('my-listings.create') }}"
       class="flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium px-4 py-2.5 rounded-xl transition-colors">
...
<div class="flex gap-1 bg-slate-100 p-1 rounded-xl mb-5 w-fit">
    ...
    @foreach($statusConfig as $statusKey => $cfg)
        <a href="{{ route('my-listings.index', ['status' => $statusKey]) }}"
           class="px-4 py-2 text-sm font-medium rounded-lg transition-colors
                  {{ request('status') === $statusKey ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
            {{ $cfg['label'] }}
        </a>
    @endforeach
</div>
...
<div class="bg-white rounded-xl border border-slate-100 overflow-hidden">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-slate-100">
                        <th class="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-6 py-3.5">Объявление</th>
                        <th class="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-3.5">Тип</th>
                        <th class="text-left text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-3.5">Статус</th>
                        <th class="text-right text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-3.5">Просмотры</th>
                        <th class="text-right text-xs font-semibold text-slate-500 uppercase tracking-wider px-4 py-3.5">Дата</th>
                        <th class="text-right text-xs font-semibold text-slate-500 uppercase tracking-wider px-6 py-3.5">Действия</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50">
                    @foreach($listings as $listing)
                        @php
                            $status = $statusConfig[$listing->status->value] ?? ['label' => $listing->status->label(), 'class' => 'bg-slate-100 text-slate-500'];
                        @endphp
                        <tr class="hover:bg-slate-50 transition-colors">
                            <!-- Title + Image -->
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-11 h-11 rounded-lg overflow-hidden bg-slate-100 flex-shrink-0">
                                    ...
                                    <div class="min-w-0">
                                        <a href="{{ route('listings.show', $listing->slug) }}"
                                           target="_blank"
                                           class="font-medium text-slate-900 text-sm hover:text-primary-600 transition-colors line-clamp-1 block">
                                            {{ $listing->title }}
                                        </a>
                                        @if($listing->price)
                                ...
                                <div class="flex items-center justify-end gap-2" x-data="{ menuOpen: false }">
                                    ...
                                    @if($listing->status->value === 'active')
                                        <form method="POST" action="{{ route('my-listings.archive', $listing->slug) }}">
                                            ...
                                            @method('PATCH')
                                            <button type="submit"
                                                    class="p-1.5 text-slate-400 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg transition-colors"
                                                    title="Архивировать">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    @else
        <div class="py-16 text-center">
            ...
            <h3 class="text-lg font-semibold text-slate-900 mb-2">Нет объявлений</h3>
            <p class="text-slate-500 text-sm mb-5">Создайте своё первое объявление прямо сейчас</p>
            <a href="{{ route('my-listings.create') }}"
