...
<div class="mb-6">
    <div class="flex items-center gap-2 text-sm text-slate-500 mb-3">
        <a href="{{ route('messages.index') }}" class="hover:text-primary-600 transition-colors">Сообщения</a>
        <svg class="w-4 h-4 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/>
        </svg>
        <span class="text-slate-700 truncate">{{ $otherParticipant->name }}</span>
    </div>
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-bold text-slate-900">{{ $otherParticipant->name }}</h1>
            ...
            @foreach($messages as $message)
                ...
                <div class="flex {{ $isOwn ? 'justify-end' : 'justify-start' }}">
                    <div class="max-w-[75%] {{ $isOwn ? 'order-2' : 'order-1' }}">
                        ...
                        @if(!$isOwn)
                            <div class="flex items-end gap-2 mb-1">
                                <div class="w-6 h-6 rounded-full bg-primary-100 flex items-center justify-center flex-shrink-0">
                                    <span class="text-primary-700 font-semibold" style="font-size: 10px;">
                                        {{ substr($otherParticipant->name, 0, 1) }}
                                    </span>
                                </div>
                                <span class="text-xs text-slate-400">{{ $otherParticipant->name }}</span>
                            </div>
                        @endif
                        ...
                        <p class="text-xs text-slate-400 mt-1 {{ $isOwn ? 'text-right' : 'text-left' }}">
        ...
        @else
            ...
            <div class="flex items-center justify-center h-48">
                <div class="text-center">
                    ...
                    <p class="text-sm text-slate-500">Начните переписку</p>
                </div>
            </div>
        ...
        <form action="{{ route('messages.reply', $conversation) }}"
            ...
            <div class="flex-1">
                <textarea name="body"
                          ...
                          class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-300 resize-none bg-white @error('body') border-red-400 @enderror">{{ old('body') }}</textarea>
                @error('body')
            ...
            <button type="submit"
                    class="flex-shrink-0 inline-flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium px-5 py-3 rounded-xl transition-colors">
