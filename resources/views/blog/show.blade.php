...
@section('og_description', isset($post->excerpt) && $post->excerpt ? $post->excerpt : Str::limit(strip_tags($post->body ?? ''), 160))
@section('og_image', isset($post->image) && $post->image ? asset('storage/' . $post->image) : asset('favicon.svg'))
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-3">
            ...
            <nav class="flex items-center gap-2 text-sm text-slate-500 mb-6">
                <a href="{{ route('blog.index') }}" class="hover:text-primary-600 transition-colors">Блог</a>
                ...
                @endif
                <span class="text-slate-700 truncate max-w-xs">{{ $post->title }}</span>
            </nav>

            <!-- Article Card -->
            <article class="bg-white rounded-xl border border-slate-100 overflow-hidden">
                ...
                @else
                    <div class="h-48 bg-gradient-to-br from-primary-50 via-primary-50 to-purple-50 flex items-center justify-center">
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <div class="flex flex-wrap items-center gap-3 mb-5">
                        @if(isset($post->category) && $post->category)
                            <a href="{{ route('blog.index', ['category' => $post->category]) }}"
                               class="text-xs font-semibold text-primary-600 bg-primary-50 hover:bg-primary-100 px-3 py-1 rounded-full transition-colors">
                                {{ $post->category }}
                            </a>
                        @endif
                    ...
                    <div class="prose prose-gray max-w-none text-slate-700 leading-relaxed
                                [&>h2]:text-xl [&>h2]:font-bold [&>h2]:text-slate-900 [&>h2]:mt-8 [&>h2]:mb-4
                                [&>h3]:text-lg [&>h3]:font-semibold [&>h3]:text-slate-900 [&>h3]:mt-6 [&>h3]:mb-3
                                [&>p]:mb-4 [&>p]:text-slate-700
                                [&>ul]:mb-4 [&>ul]:list-disc [&>ul]:pl-5 [&>ul>li]:mb-1
                                [&>ol]:mb-4 [&>ol]:list-decimal [&>ol]:pl-5 [&>ol>li]:mb-1
                                [&>blockquote]:border-l-4 [&>blockquote]:border-primary-300 [&>blockquote]:pl-4 [&>blockquote]:italic [&>blockquote]:text-slate-600 [&>blockquote]:my-4
                                [&>a]:text-primary-600 [&>a]:underline [&>a]:hover:text-primary-700
                    ...
                    <div class="mt-8 pt-6 border-t border-slate-100">
                        <div class="flex items-center gap-3">
                            <span class="text-sm font-medium text-slate-500">Поделиться:</span>
                            <a href="https://t.me/share/url?url={{ urlencode(request()->url()) }}&text={{ urlencode($post->title) }}"
                               target="_blank" rel="noopener noreferrer"
                               class="w-9 h-9 bg-primary-500 hover:bg-primary-600 text-white rounded-full flex items-center justify-center transition-colors">
            ...
            @if(isset($related) && $related->count())
                <div class="mt-8">
                    <h2 class="text-xl font-bold text-slate-900 mb-5">Похожие статьи</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        @foreach($related as $relatedPost)
                            <a href="{{ route('blog.show', $relatedPost->slug) }}"
                               class="bg-white rounded-xl border border-slate-100 p-5 hover:shadow-md hover:border-slate-200 transition-all group">
                                ...
                                <h3 class="font-semibold text-slate-900 group-hover:text-primary-600 transition-colors leading-snug line-clamp-2">
                                    {{ $relatedPost->title }}
                                </h3>
                            </a>
                        @endforeach
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            @if(isset($post->category) && $post->category)
                <div class="bg-white rounded-xl border border-slate-100 p-5">
                    <p class="text-xs text-slate-400 uppercase tracking-wide mb-2">Категория</p>
                    <a href="{{ route('blog.index', ['category' => $post->category]) }}"
